...
class AttentionPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.reshape(x.shape[0], x.shape[1], x.shape[2]
                      * x.shape[3]).permute(2, 0, 1)
        x = torch.cat([x.mean(dim=0, keepdim=True), x],
                      dim=0)  # (HW+1)NC  50,32,2048
...
class ModifiedResNet(nn.Module):
    ...
    def __init__(self, layers, output_dim, heads, input_resolution=224, width=64):
        ...
        self.attnpool = AttentionPool2d(
            input_resolution, embed_dim, heads, output_dim)
...
def build_model(state_dict: dict, h_resolution: int, w_resolution: int, vision_stride_size: int):
    ...
    if vit:
        ...
        state_dict["visual.attnpool.positional_embedding"] = resize_pos_embed(
            state_dict["visual.attnpool.positional_embedding"], model.visual.attnpool.positional_embedding, h_resolution, w_resolution)
...
def resize_pos_embed(posemb, posemb_new, hight, width):
    ...
    print('Resized position embedding: %s to %s',
